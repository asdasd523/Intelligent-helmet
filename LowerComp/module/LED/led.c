...
static ssize_t led_chr_dev_read(struct file *filp, char __user *buf, size_t cnt, loff_t *offt)
{
	...
	if ((read_flag = gpio_get_value(rgb_led_green)) >= 0)
		...
		printk(KERN_EMERG "\t  read green_led error  \n");
		return -3;
	}
	if ((read_flag = gpio_get_value(rgb_led_blue)) >= 0)
		read_data |= !read_flag;
	else{
		printk(KERN_EMERG "\t  read blue_led error  \n");
		return -2;
	}
	...
	//printk(KERN_EMERG "\t  read led successful  \n");
	return 1;
}
