...
struct dht11_dev {
    ...
    struct class *dht11_class;       //dht11设备类
    struct device_node	*dev_nd;     /* device node */ //用于保存设备树上的节点信息
    dev_t dev_num;                   //设备号
    int major;                       //主设备号
    int minor;                       //从设备号
    int dht11_pin;                   //gpio引脚
...
static void get_tempAndHum(void)  {      
    int j = 0;  	    
    gpio_direction_output(dht11_dev->dht11_pin, LOW_LEVEL);      //设置输出低电平
    mdelay(20);      
    gpio_direction_output(dht11_dev->dht11_pin, HIGH_LEVEL);     //设置输出高电平
    udelay(40);  	    
    gpio_direction_input(dht11_dev->dht11_pin);                  //设置为输入模式
...
dht11_probe(struct platform_device *pdev)
{
    ...
    //2.注册dht11字符设备
    dht11_dev->dev_num = MKDEV(dht11_dev->major, 0);     //组成设备号
    if (dht11_dev->major) {	/*  defined device number */
		result = register_chrdev_region(dht11_dev->dev_num, DEV_COUNT, DEV_NAME);   //注册设备号
    ...
    cdev_init(&dht11_dev->cdev, &dht11_fops);    //关联字符设备与文件操作
    dht11_dev->cdev.owner = THIS_MODULE;
    dht11_dev->cdev.ops = &dht11_fops;
    result = cdev_add(&dht11_dev->cdev, dht11_dev->dev_num, 1);   //添加设备至cdev_map哈系表中
    if (result)
    {
    ...
    dht11_dev->dht11_class = class_create(THIS_MODULE, "dht11_class");  //创建设备类
    ...
    //3.由设备树获取硬件信息,并申请gpio
	// printk(KERN_EMERG "of_find_node_by_path OK \r\n");
#ifdef USE_DHT11
    dht11_dev->dev_nd = of_find_node_by_path("/dht11");      //获取设备树上的节点信息
...
#endif
    spin_lock_init(&dht11_lock);  //初始化自旋锁
...
fail0:
    kfree(dht11_dev);                          //设备结构体指针申请空间失败，释放全部空间
...
{
    gpio_free(dht11_dev->dht11_pin);      //释放申请的gpio引脚
