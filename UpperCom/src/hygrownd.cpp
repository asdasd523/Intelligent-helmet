...
HygroWnd::HygroWnd(QWidget *parent) : QWidget(parent)
{
    m_width  = 20;         //宽度
    maxValue = 85;        //量程
    minValue = 5;
...
void HygroWnd::Update_value(qreal value){
...
void HygroWnd::startAnimation(){
    ...
    m_valueAnimation->setKeyValueAt(0.5,startValue-1);
    m_valueAnimation->setKeyValueAt(1  ,startValue  );
    m_valueAnimation->start();

}

void HygroWnd::paintEvent(QPaintEvent *e) {
     //qDebug() << "paint event";
     updateRect();                  //更新矩形图案
     ...
     painter.fillRect(tmpRect, QColor(0, 0, 255));
     ...
     if(h > m_rect.height())
         h = m_rect.height();
     painter.fillRect(m_rect.adjusted(0, m_rect.height()-h-perHeight/2-1 , 0, 0), QColor(0, 0, 255));
}
